...
import com.foreveross.infra.test.tester.H2DataSourceDatabaseTester;
import com.foreveross.infra.test.tester.MySQLDataSourceDatabaseTester;
import com.foreveross.infra.test.tester.OracleDataSourceDatabaseTester;
...
public abstract class Dbunit {
	...
	public Dbunit() {
		...
		if (getDataSource() != null) {
			...
			try {
				DataSource ds = setupDataSource(PropertiesUtil.JDBC_DRIVER,
						PropertiesUtil.JDBC_USERNAME,
						PropertiesUtil.JDBC_PASSWD, jdbc_url);
				if (PropertiesUtil.JDBC_DRIVER.toLowerCase().contains("mysql")) {
					logger.info("use mysql datasource tester.");
	...
	protected DataSource setupDataSource(String driver, String username,
