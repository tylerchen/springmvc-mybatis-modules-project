...
public class MybatisSqlSessionFactoryBean implements
			...
			for (Resource mapperLocation : this.mapperLocations) {
				...
				try {
					XMLMapperBuilder xmlMapperBuilder = new XMLMapperBuilder(
							...
							mapperLocation.toString(), configuration
									.getSqlFragments());
	...
	private void autoTypeAliases(Configuration configuration)
			throws IOException {
		...
		while (urls.hasMoreElements()) {
			...
			if ("file".equals(protocol) || "jar".equals(protocol)) {
					...
					for (Enumeration<?> entries = jarFile.entries(); entries
						...
						if (entryName.startsWith(basePackage)
							...
							entryName = entryName.substring(
									entryName.indexOf(basePackage)).replaceAll(
									"\\\\", "/");
							lists.add(entryName);
						}
					}
				...
				String jarPath = filePath.substring(0, filePath
						.indexOf("contents"));
		...
		for (String className : lists) {
			if (className.startsWith("com/foreveross/infra/test")) {
				continue;
			}
			try {
				...
				String upperCase = c.getSimpleName().toUpperCase();
				if (upperCase.endsWith("DTO") || upperCase.endsWith("VO")
						|| upperCase.endsWith("Repository") && c.isInterface()) {
					configuration.getTypeAliasRegistry().registerAlias(c);
				}
	...
	private void autoParse(Configuration configuration) throws IOException {
		...
		if (scanMappingResourceDir == null
				|| scanMappingResourceDir.trim().length() < 1) {
			return;
		}
		List<String> lists = new ArrayList<String>();
		Enumeration<?> urls = Thread.currentThread().getContextClassLoader()
				.getResources(scanMappingResourceDir);
		while (urls.hasMoreElements()) {
			URL u = (URL) urls.nextElement();
			String path = u.getFile();
			...
			if ("file".equals(protocol) || "jar".equals(protocol)) {
				if (path.endsWith("!/" + scanMappingResourceDir)) {
					...
					for (String fileName : files) {
						lists.add(scanMappingResourceDir + File.separator
								+ fileName);
					}
				}
			} else if ("vfs".equals(protocol)) {
